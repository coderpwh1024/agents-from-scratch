...
| --- |--------------------------------------------|------------------|
| `agent_system_prompt` |   “使用 write_email 工具起草回复邮件”；调用后任务完成。                   |  文案称“起草”，本身不保证真实发送。|
| `AGENT_TOOLS_PROMPT` |  write_email(to, subject, content) - Send emails to specified recipients。 | 是，明确表示向指定收件人发送邮件。|
| `write_email` 的 docstring 与实现 | Write and send an email.；返回 Email sent to ...。                                 |  是；当前是占位实现，但真实接入时会直接产生发送副作用        |
...
| 项目 | 你的设计                    |
| --- |-------------------------|
| 输入 schema | to,from,subject,content |
| 哪些信息必须完整 | 邮件的接收者，邮件的发送者，邮件的标题，邮件的正文          |
| 允许直接执行的条件 |                         |
| 必须暂停并请求人工审批的条件 |                         |
| 缺少信息时的行为 |                         |
| 工具执行失败时的行为 |                         |
